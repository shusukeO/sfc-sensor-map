...
SoxClient.prototype.connect = function () {
  ...
  var callback = function (status) {
    if (status == Strophe.Status.CONNECTING) {
      ...
      me.connection.PubSub.bind(
        'xmpp:pubsub:last-published-item',
        function (obj) {
          try {
            me._processLastPublishedItem(
              obj.node,
              obj.id,
              obj.entry,
              obj.timestamp
      ...
      if ($.browser.msie) {
        ...
        for (var i = 0; i < element.length; i++) {
          ...
          for (var j = 0; j < element[i].attributes.length; j++) {
            dumphtml.push(
              ' ' +
                element[i].attributes[j].nodeName +
                '="' +
...
SoxClient.prototype.deleteDevice = function (device) {
  ...
  this.connection.PubSub.deleteNode(device.nodeName + '_data')
    .done(successDataCallback)
    .fail(failureCallback);
...
SoxClient.prototype.discoverDevices = function (query) {
  ...
  // }else{
  this.connection.PubSub.discoverNodes()
    .done(successCallback)
    .fail(failureCallback);
  // }
  return true;
...
SoxClient.prototype.publishDevice = function (device) {
  ...
  if (device.isDataDirty()) {
    this.connection.PubSub.publish(
      device.nodeName + '_data',
      $build('data').t(device.toDataString()).tree(),
      device.nodeName + '_data'
    )
      .done(successDataCallback)
      .fail(failureCallback);
  ...
  if (device.isMetaDirty()) {
    this.connection.PubSub.publish(
      device.nodeName + '_meta',
      $build('device', {
        name: device.name,
        type: device.type,
      })
        .t(device.toMetaString())
        .tree(),
      'metaInfo'
    )
      .done(successMetaCallback)
...
SoxClient.prototype.subscribeDevice = function (device) {
  ...
  var successMetaCallback = function (data) {
    ...
    me.connection.PubSub.subscribe(device.nodeName + '_data')
      .done(successDataCallback)
      .fail(failureCallback);
  };
  var failureCallback = function (data) {
   ...
   * node
   */
  this.connection.PubSub.subscribe(device.nodeName + '_meta')
    .done(successMetaCallback)
    .fail(failureCallback);
...
SoxClient.prototype.unsubscribeDevice = function (device) {
  if (
    !this.isConnected() ||
    (!device.metaSubscribed && !device.dataSubscribed)
  ) {
    return false;
  }
  ...
  var successDataCallback = function (data) {
    ...
    this.connection.PubSub.unsubscribe(device.nodeName + '_meta')
      .done(successMetaCallback)
      .fail(failureCallback);
  };
  var failureCallback = function (data) {
  ...
  this.connection.PubSub.unsubscribe(device.nodeName + '_data')
    .done(successDataCallback)
    .fail(failureCallback);
...
SoxClient.prototype.unsubscribeAll = function () {
  ...
  var successCallback = function (data) {
    ...
    for (var i = 0; i < data.length && i < 100; i++) {
      ...
      me.connection.PubSub.unsubscribe(data[i].node, data[i].jid, data[i].subid)
        .done(function () {
          var now = new Date();
  ...
  // first subscribe _data node
  this.connection.PubSub.getSubscriptions()
    .done(successCallback)
    .fail(failureCallback);
 ...
 */
SoxClient.prototype._processLastPublishedItem = function (
  node,
  id,
  entry,
  timestamp
  ...
  if (node.indexOf('_meta') != -1) {
    ...
    if (this.soxEventListener) {
      this.soxEventListener.metaDataReceived({
        soxClient: this,
